...
- Usemos datos sobre salarios ([salarios.csv]((/diapositivas/salarios.csv)) de 175 mujeres para ejemplificar la construcción del histograma 
...
```{r out.width="100%", echo=F, include=F, message=F, warning=F}
data.salarios<-read_csv("salarios.csv",
                        locale = locale(encoding = "latin1"))
```
...
```{r echo=T, eval=F, message=T, warning=F}
data.salarios %>% 
  ggplot(aes(x=lnhwage)) +
  geom_histogram(aes(y=..density..),
                 bins=20,
...
```{r echo=T, include=T, eval=T, results=F, message=T, warning=F}
...
.pull-left[
```{r out.width="100%", echo=T, eval=F, message=F, warning=F}
data.sim %>% 
  ggplot(aes(x=x,y=y))+
  geom_point()+
...
.pull-right[
```{r out.width="100%", echo=F, include=T, eval=T, results=T, message=F, warning=F, fig.height=6}
data.sim %>% 
  ggplot(aes(x=x,y=y))+
  geom_point()+
  geom_smooth(aes(x=x, y=y, color='MCO'),
...
.pull-left[
```{r out.width="100%", echo=T, include=T, eval=F, results=T, message=F, warning=F}
...
.pull-right[
```{r out.width="100%", echo=F, include=T, eval=T, results=T, message=F, warning=F, fig.height=4}
data.sim %>% 
  ggplot(aes(x=x, y=y))+
  geom_point()+
  geom_smooth(aes(x=x, y=y, color='MCO cúbico'),
...
```{r out.width="100%", echo=F, include=T, eval=T, results=T, message=F, warning=F, cache=T}
par(mar = c(1, 1, 1, 1))
...
.pull-right[
```{r out.width="100%", echo=T, include=T, eval=T, results=F, message=F, warning=F, fig.height=6}
r.graph
```
]
